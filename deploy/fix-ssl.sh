#!/bin/bash
echo "=== Fix SSL for EcoMarket ==="
SOCKET=$(find /run/php/ -name "*.sock" 2>/dev/null | head -1)

# Step 1: HTTP-only config so nginx works
cat > /etc/nginx/sites-available/mercadolibre << ENDNGINX
server {
    listen 80;
    server_name mercadolibre.centralchat.pro;
    root /var/www/mercadolibre;
    index index.html index.php;
    location / { try_files \$uri \$uri/ =404; }
    location ~ \.php\$ {
        include snippets/fastcgi-php.conf;
        fastcgi_pass unix:${SOCKET};
        fastcgi_param SCRIPT_FILENAME \$document_root\$fastcgi_script_name;
        include fastcgi_params;
    }
    location /data/ { deny all; }
    location ~ /\.git { deny all; }
}
ENDNGINX

ln -sf /etc/nginx/sites-available/mercadolibre /etc/nginx/sites-enabled/mercadolibre
nginx -t 2>&1 && systemctl reload nginx && echo "HTTP OK"

# Step 2: Get SSL
certbot --nginx -d mercadolibre.centralchat.pro --non-interactive --agree-tos --email admin@centralchat.pro --redirect 2>&1

# Step 3: Fix le_http challenge file if needed
touch /etc/letsencrypt/le_http_01_cert_challenge.conf 2>/dev/null
nginx -t 2>&1 && systemctl reload nginx && echo "NGINX FINAL OK"

# Step 4: Permissions
chown -R www-data:www-data /var/www/mercadolibre/data /var/www/mercadolibre/uploads
chmod 775 /var/www/mercadolibre/data /var/www/mercadolibre/uploads

# Step 5: Test
curl -sk https://mercadolibre.centralchat.pro/api/test.php 2>&1 || curl -s http://mercadolibre.centralchat.pro/api/test.php 2>&1
echo ""
echo "=== DONE ==="
